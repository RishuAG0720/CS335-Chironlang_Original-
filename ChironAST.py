...
class AssignmentCommand(Instruction):
    ...
    def __str__(self):
        if isinstance(self.rexpr, PhiFunction):
            return f"{self.lvar} = {self.rexpr}"
        else:
            return f"{self.lvar} = {self.rexpr}"
...
class PhiFunction(Expression):
    def __init__(self, target_var, source_vars):
        """
        Represents a Phi function in SSA form
        
        Parameters:
        - target_var: The variable that gets the result of the Phi function
        - source_vars: List of source variables that go into the Phi function
        """
        self.target_var = target_var
        self.source_vars = source_vars

    def __str__(self):
        sources_str = ", ".join([f"{var[0]}" if isinstance(var, tuple) else f"{var}" 
                                for var in self.source_vars])
        return f"Φ({sources_str})"
