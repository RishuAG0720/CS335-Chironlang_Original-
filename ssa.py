...
# #         """
# #         for var_name in set(self.variable_definitions.keys()) | set(self.variable_uses.keys()):
# #             self.counter[var_name] = 0
# #             self.stack[var_name] = []
...
# #         for node, idom in self.dom_tree.items():
# #             dom_children[idom].append(node)
        
# #         self._rename_block(entry, dom_children)
    
# #     def _rename_block(self, block, dom_children):
# #         """
# #         Rename variables in a single block and its dominance children
# #         """
# #         old_stacks = {var: list(stack) for var, stack in self.stack.items()}
        
# #         if hasattr(block, 'phi_functions'):
# #             for phi in block.phi_functions:
# #                 var_name = phi.target_var.varname
# #                 self.counter[var_name] += 1
# #                 new_name = f"{var_name}_{self.counter[var_name]}"
# #                 phi.target_var = Var(new_name)
# #                 self.stack[var_name].append(self.counter[var_name])
        
# #         new_instrlist = []
# #         for idx, (instr, line_num) in enumerate(block.instrlist):
# #             new_instr = copy.deepcopy(instr)
            
# #             if isinstance(new_instr, AssignmentCommand):
# #                 # Rename uses in the right-hand side first
# #                 new_instr.rexpr = self._rename_expr(new_instr.rexpr)
                
# #                 # Generate new name for the defined variable
# #                 var_name = new_instr.lvar.varname
# #                 self.counter[var_name] += 1
# #                 new_name = f"{var_name}_{self.counter[var_name]}"
# #                 new_instr.lvar = Var(new_name)
                
# #                 # Push new version onto stack
# #                 self.stack[var_name].append(self.counter[var_name])
                
# #                 # For repeat counters, ensure version is tracked
...
# #                 new_instr.cond = self._rename_expr(new_instr.cond)
# #             elif hasattr(new_instr, 'expr'):
# #                 new_instr.expr = self._rename_expr(new_instr.expr)
            
# #             new_instrlist.append((new_instr, line_num))
        
# #         block.instrlist = new_instrlist
        
# #         # Add sources to phi functions in successor blocks
# #         for succ in self.cfg.successors(block):
...
# #                     if var_name in self.stack and self.stack[var_name]:
# #                         current_version = self.stack[var_name][-1]
# #                         source_var = Var(f"{var_name}_{current_version}")
                        
# #                         existing_sources = [s for s, e in phi.source_vars if e == edge_label]
# #                         if not existing_sources:
# #                             phi.source_vars.append((source_var, edge_label))
        
# #         for child in dom_children[block]:
# #             self._rename_block(child, dom_children)
        
# #         self.stack = old_stacks
    
# #     def _rename_expr(self, expr):
# #         """Rename variables in expressions to their SSA form"""
# #         if isinstance(expr, Var):
# #             var_name = expr.varname
...
# #         elif hasattr(expr, 'lexpr') and hasattr(expr, 'rexpr'):
# #             expr.lexpr = self._rename_expr(expr.lexpr)
# #             expr.rexpr = self._rename_expr(expr.rexpr)
# #             return expr
# #         elif hasattr(expr, 'expr'):
...
#                     self.repeat_handler.increment_version(var_name)
#             elif hasattr(new_instr, 'cond'):
#                 # Bug: Previously, condition variables were not renamed, causing use of unversioned :__rep_counter_1
#                 # Fix: Rename variables in conditions to their latest SSA version
#                 new_instr.cond = self._rename_expr(new_instr.cond)
#             elif hasattr(new_instr, 'expr'):
#                 new_instr.expr = self._rename_expr(new_instr.expr)
...
#                 return Var(f"{var_name}_{current_version}")
#             else:
#                 # Bug: Incorrect default versioning for repeat counters led to :_0, :_1 in Phi sources
#                 # Fix: Use repeat_handler to get correct version for repeat counters
#                 if self.repeat_handler.is_repeat_counter(var_name):
#                     current_version = self.repeat_handler.get_current_version(var_name)
#                     if current_version > 0:
#                         return Var(f"{var_name}_{current_version}")
...
#                 # Bug: Incorrect Phi sources for repeat counters (e.g., :_0, :_1)
#                 # Fix: Rebuild Phi sources using the latest version from each predecessor
#                 if self.repeat_handler.is_repeat_counter(base_var_name):
#                     phi.source_vars = []
